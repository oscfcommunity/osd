...
export default function VenueGallery() {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h2 className="text-2xl md:text-3xl font-bold text-green-600 mb-6 md:mb-8">Venue Photos</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {images.map((img, index) => (
          <figure key={img.src} className="group overflow-hidden rounded-2xl border border-gray-100 shadow-sm bg-white hover:shadow-md transition-shadow">
            ...
            <figcaption className="px-3 py-2 text-sm text-gray-700">{img.caption}</figcaption>
          </figure>
        ))}
      </div>
      ...
      {/* Lightbox */}
      <div
        className={`fixed inset-0 z-50 ${isOpen ? "flex" : "hidden"} items-center justify-center bg-white/70 backdrop-blur-sm`}
        role="dialog"
        aria-modal="true"
        ...
        <div className="flex flex-col items-center">
          {isOpen && (
            <div className="relative w-full h-[72vh] bg-white rounded-xl shadow-xl border border-gray-100 flex items-center justify-center overflow-hidden">
              <img src={active.src} alt={active.alt} className="max-w-full max-h-full object-contain" />
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
